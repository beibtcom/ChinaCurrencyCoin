...
#else
    const char* pszModule = "ChinaCurrencyCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ChinaCurrencyCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ChinaCurrencyCoin
    // Mac: ~/Library/Application Support/ChinaCurrencyCoin
    // Unix: ~/.ChinaCurrencyCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ChinaCurrencyCoin";
#else
    fs::path pathRet;
    ...
    return pathRet / "ChinaCurrencyCoin";
#else
    // Unix
    return pathRet / ".ChinaCurrencyCoin";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ChinaCurrencyCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ChinaCurrencyCoind.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong ChinaCurrencyCoin will not work properly.");
                    strMiscWarning = strMessage;
                    printf("*** %s\n", strMessage.c_str());
                    uiInterface.ThreadSafeMessageBox(strMessage+" ", string("ChinaCurrencyCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION);
                }
            }
        }
        if (fDebug) {
