...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("ChinaCurrencyCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #ChinaCurrencyCoinTEST2\r");
            Send(hSocket, "WHO #ChinaCurrencyCoinTEST2\r");
        } else {
            // randomly join #ChinaCurrencyCoin00-#ChinaCurrencyCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #ChinaCurrencyCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #ChinaCurrencyCoin%02d\r", channel_number).c_str());
        }
