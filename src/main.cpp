...
unsigned int nStakeMinAge = 8 * nOneHour; 
unsigned int nStakeMaxAge = 60 * 24 * nOneHour;
...
const string strMessageMagic = "ChinaCurrencyCoin Signed Message:\n";
...
{
    int64_t nRewardCoinYear = 0.1 * CENT;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (pindexLast == NULL || pindexLast->nHeight < 101)
        return bnTargetLimit.GetCompact(); // genesis block + first 100
...
void ThreadScriptCheck(void*) {
    ...
    RenameThread("ChinaCurrencyCoin-scriptch");
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in ChinaCurrencyCoin
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (fProofOfStake)
    {
        ...
        // ChinaCurrencyCoin: check proof-of-stake block signature
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ChinaCurrencyCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "The Times 07202017 CS is doing his research work from UCD";
        CTransaction txNew;
        txNew.nTime = 1499274690;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1499274690;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1755518; // !fTestNet ? 1575379 : 46534;
