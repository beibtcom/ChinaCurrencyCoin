ChinaCurrencyCoin-qt: Qt4 GUI for ChinaCurrencyCoin
===============================
...
Alternatively, install Qt Creator and open the `ChinaCurrencyCoin-qt.pro` file.

An executable named `ChinaCurrencyCoin-qt` will be built.
...
A warning for people using the *static binary* version of ChinaCurrencyCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of ChinaCurrencyCoin is linked against libdb5.3.
...
installed causes ChinaCurrencyCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
